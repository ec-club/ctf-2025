...
void urandom(void *data, int data_size)
{
...
// See: https://github.com/ai/nanoid/blob/main/index.js
static const char alphabet[] = "useandom-26T198340PX75pxJACKVERYMINDBUSHWOLF_GQZbfghjklqvwyzrict";
void nanoid(char *s, size_t size)
{
  char bytes[size];
  urandom(bytes, size);
  for (size_t i = 0; i < size; i++)
  {
    s[i] = alphabet[bytes[i] & 63];
  }
  s[size - 1] = '\0';
}
...
void generate_flag(char *template, void *target)
{
  if (!template)
  {
    puts("No flag was provided. Please create a ticket in Discord.");
    exit(1);
  }
  char alpha[10];
  char beta[10];
  nanoid(alpha, sizeof(alpha));
  nanoid(beta, sizeof(beta));
  sprintf(target, template, alpha, beta);
}

const int PROOF_SIZE = 3;
const int SHELLCODE_SIZE = 0x1000;
void main(int argc, char *argv[])
{
  char flag[256];
  generate_flag(getenv("FLAG"), flag);
