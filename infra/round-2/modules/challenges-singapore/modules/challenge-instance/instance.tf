...
# Let's not mess up with external network access of the challenge servers, as they need to access external AI APIs.
resource "aws_eip" "challenge-server" {
  domain   = "vpc"
  instance = aws_instance.challenge-server.id

  associate_with_private_ip = aws_network_interface.challenge-server.private_ip
  tags = {
