variable "vpc_id" {
  description = "The ID of the VPC"
  type        = string
}
variable "vpc_ipv4_cidr_block" {
  description = "The IPv4 CIDR block for the VPC"
  type        = string
}
variable "vpc_ipv6_cidr_block" {
  description = "The IPv6 CIDR block for the VPC"
  type        = string
}
variable "public_route_table_id" {
  description = "The ID of the public routing table"
  type        = string
}

variable "ctfd_subnet_index" {
  description = "Index of the subnet for CTFd"
  type        = number
}
variable "ctfd_internal_subnet_indices" {
  description = "List of indices for internal subnets for CTFd"
  type        = list(number)
}

variable "vpc_endpoint_sg_ids" {
  description = "List of security group IDs for VPC endpoints"
  type        = list(string)
}
variable "vpc_endpoint_cidr_block" {
  description = "CIDR block for VPC endpoint access"
  type        = string
}

variable "instance_connect_sg_ids" {
  description = "List of security group IDs for EC2 Instance Connect"
  type        = list(string)
}
variable "s3_cidr_blocks" {
  description = "List of CIDR blocks for S3 VPC endpoint"
  type        = list(string)
}

variable "letsencrypt_ca_server" {
  description = "The Let's Encrypt CA server URL"
  type        = string
}

variable "ec2_key_pair_name" {
  description = "The name of the EC2 key pair"
  type        = string
}
variable "assume_policy_ec2" {
  description = "The assume role policy for EC2"
  type        = string
}

variable "dns_zone_id" {
  description = "The Route53 hosted zone ID"
  type        = string
}
variable "dns_zone_name" {
  description = "The Route53 hosted zone name"
  type        = string
}
variable "internal_dns_zone_name" {
  description = "The internal DNS zone name"
  type        = string
}
variable "internal_dns_zone_id" {
  description = "The internal DNS zone ID"
  type        = string
}
