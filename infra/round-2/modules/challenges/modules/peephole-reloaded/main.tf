...
module "challenge-instance" {
  ...
  instance_profile_name = aws_iam_instance_profile.peephole-reloaded.name

  vpc_id                 = var.vpc_id
  subnet_id              = var.subnet_id
  subnet_ipv6_cidr_block = var.subnet_ipv6_cidr_block
  sg_id                  = aws_security_group.peephole-reloaded.id
  ...
  compose_file          = file("../../round-2/pwn/peephole-reloaded/docker-compose.yml")
  custom_script         = <<EOF
if ! grep -q "net.core.rmem_max=7500000" /etc/sysctl.conf; then
  echo "net.core.rmem_max=7500000" | tee -a /etc/sysctl.conf
  echo "net.core.wmem_max=7500000" | tee -a /etc/sysctl.conf
  sysctl -p
fi
EOF

  dns_names = ["peephole-reloaded"]
}
