...
resource "aws_eip" "ctfd" {
  domain            = "vpc"
  network_interface = aws_network_interface.ctfd.id
  tags = {
    Name = "CTFd public IP"
  }
}
...
resource "aws_instance" "ctfd" {
  ...
  export TLS_CERTS_PATH=$MOUNT_POINT
	...
	export DOMAIN=${aws_route53_zone.ctf.name}
  ...
  sed -i "s/MONITORING_DOMAIN/${aws_route53_zone.internal.name}/g" /opt/ctfd/config.alloy
	cd /opt/ctfd
...
resource "aws_route53_record" "ctfd-public" {
  zone_id = aws_route53_zone.ctf.zone_id
  name    = aws_route53_zone.ctf.name
  type    = "A"
  ttl     = 300
  records = [aws_eip.ctfd.public_ip]
}
